...
readCfg = () => {
    if (!fs.existsSync(app.getAppPath() + '/.conf')) fs.mkdirSync(app.getAppPath() + '/.conf');
    if (!fs.existsSync(app.getAppPath() + '/.conf/config.json')) {
        cfg = defaultCfg;
        win.setSize(parseFloat(cfg.width), parseFloat(cfg.height));
        writeCfg();
    } else {
        cfg = JSON.parse(fs.readFileSync(app.getAppPath() + '/.conf/config.json'));
    }
};
...
writeCfg = () => {
    ...
    fs.writeFileSync(app.getAppPath() + '/.conf/config.json', JSON.stringify(cfg));
};
